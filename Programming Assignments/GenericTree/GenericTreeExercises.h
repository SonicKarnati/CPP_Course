...
static void treeFactory(GenericTree<int>& tree) {
  ...
  auto ptr_8 = root->addChild(8);
  ptr_8->addChild(16)->addChild(42);
  ptr_8->addChild(23);
  root->addChild(15);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  if (rootNodePtr == nullptr) return results;
  //      *****************************************************
  //                           EXERCISE 2
  //    TODO: Your work here! You should edit this function body!
  //      *****************************************************
  ...
  // Running until the whole queue is empty
  while (!q.empty()){
    // Set first node to front of queue
    TreeNode* node = q.front();
    // loop through the children of that node
    for (auto i = node->childrenPtrs.begin(); i != node->childrenPtrs.end(); i++){
      // Pushing data of children
      T currentdata = (*i)->data;
      results.push_back(currentdata);
      q.push(*i);
    }
    // Clearing queue
    q.pop();
